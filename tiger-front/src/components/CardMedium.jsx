...
const CardMedium = ({articleID, setArticle, removeArticle, editMode, className = ''}) => {
    ...
    // Otherwise, display the article
        ...
        <div className={` relative h-full border-gray-200 p-3 ${className}`}>
            <Link to={`/articles/${articleID}`} className="absolute inset-0 z-10"/>
            {editMode && (
            <button onClick={removeArticle} className = "z-50 absolute top-2 right-2 text-red-500 hover:text-red-700 font-bold">
                ×
            </button>
            )}
